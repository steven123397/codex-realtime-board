...
- `apps/bridge` 已具备 mock / live 双路径、board-managed session registry、`/api/sessions`、`/api/state`、`/api/state/sync`、`/api/session/start`、`/api/session/attach` 和多会话状态归一化能力；其中 `state` / `sync` 读取面已明确区分 `no_session_selected` 与 `session_not_found` 两类空目标语义。
- `apps/panel` 已能按目标 `sessionId` 消费 bridge snapshot，并优先通过 cursor 条件同步端点减少无变化轮询；当前 companion 视图支持 stale / reconnect 反馈、运行中会话切换，以及会话缺失 / 未选定时的明确空态；当 `sync` 端点已经明确给出空态时，panel 不再额外回退一轮完整 snapshot 请求；bridge 完全不可用时再回退到 demo state。
