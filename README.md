...
- `apps/panel` 已能按目标 `sessionId` 消费 bridge snapshot，并通过定时轮询 + 手动刷新推进到更完整的 companion 视图：支持 stale / reconnect 反馈、运行中会话切换，以及会话缺失 / 未选定时的明确空态；bridge 完全不可用时再回退到 demo state。
