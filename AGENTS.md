...
- `apps/bridge` 的 mock / live 双路径、`app-server` 客户端、多会话状态归一化，以及 `/healthz`、`/api/sessions`、`/api/state`、`/api/state/sync`、`/api/session/start`、`/api/session/attach` HTTP 出口。
- `apps/panel` 对 bridge snapshot 的按会话消费路径、定时轮询 / 手动刷新、cursor 条件同步、stale / reconnect 反馈、运行中会话切换、demo fallback，以及会话缺失 / 未选定时的明确空态。
