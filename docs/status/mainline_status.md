...
- `apps/panel` 已能根据 URL 中的目标 `sessionId` 与 `bridgeUrl` 轮询指定快照，并同步读取 `/api/sessions` 暴露运行中会话目录；当前支持最近同步时间、手动刷新、stale / reconnect 反馈，以及运行中会话切换。bridge 完全不可用时回退 demo state，目标会话缺失或未选定时转为明确空态。
...
- `apps/panel` 当前虽然已经有 stale / reconnect 反馈和运行中会话切换，但仍停留在固定间隔 snapshot 拉取，还没有 bridge 侧 cursor / delta / push 形态的增量刷新合同。
- `Codex app-server` 协议仍带实验性质，后续仍需持续防守事件名和字段形态的演进风险。

## 下一步

1. 评估 bridge 侧的增量刷新合同，例如 cursor / delta 端点或 SSE / WebSocket 推送升级，但仍保持 panel 只消费 bridge 合同而不直连底层协议。
